...
import { getTokenFromBearer, verifyJwtToken } from "@/lib/helper/generateJWT";
import { checkAuth } from "@/lib/helper/auth";
...
export async function POST(
  ...
  try {
    ...
    const [schmea]: any = await pool.execute(
      "SELECT * FROM `scheme` WHERE primary_id = ?",
      [endpoint[0]?.primary_id],
    );
    ...
    const { sql, values }: any = await generateInserSql(
      request,
      userinfo[2] + userinfo[1],
    );
...
export async function PUT(
  ...
  try {
    ...
    const request = await Request.json();
    const { sql, values }: any = await generateUpdateSql(
      request,
      userinfo[2] + userinfo[1],
      userinfo[3],
    );
...
export async function DELETE(
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `DELETE FROM ${userinfo[2] + userinfo[1]} WHERE primary_id=?`,
      [userinfo[3]],
    );
    return MyResponse(
      {
