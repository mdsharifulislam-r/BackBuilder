...
export default function DeletePopUp({name,setHide}:{name:string,setHide:React.Dispatch<React.SetStateAction<boolean>>}) {
  ...
  return (
    <div className='fixed w-full h-full left-0 top-0 flex justify-center place-items-center z-20'>
        ...
        <div className="relative md:w-[30%] w-[80%] bg-white min-h-44 z-30 rounded-lg p-2">
            <div className='text-center py-3'>
                <h1 className='text-2xl text-primary'>Are you sure delete this <span className='text-orange'>{name}?</span></h1>
            ...
            <div className='py-4 flex justify-center place-items-center gap-3 '>
                <button onClick={()=>setHide(prev=>!prev)} className='bg-primary shadow-card px-3 py-2 rounded-lg text-white'>Cencel</button>
                <LoadingButton isLoading={Loading} onClick={deleteEnd} className='bg-red-600 shadow-card px-3 py-2 rounded-lg text-white'>Delete</LoadingButton>
            </div>
        </div>
