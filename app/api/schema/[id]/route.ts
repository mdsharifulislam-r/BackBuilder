...
export async function DELETE(Request:Request,{params}:{params:{id:string}}) {
    try {
        ...
        const [datas]= await DbPool.query(`ALTER TABLE ${endpoint_name+project_id} DROP COLUMN ${schma_name}`)
...
export async function POST(Request:Request) {
    try {
        ...
        await pool.query('CREATE TABLE IF NOT EXISTS scheme (schema_id int not NULL AUTO_INCREMENT PRIMARY KEY,primary_id INT,name varchar(256),type varchar(256),required boolean)')
        ...
        const [rowss]:any= await pool.query('SELECT * FROM `scheme` WHERE primary_id=? AND name=?',[primary_id,name])
        if(rowss?.length){
            return NextResponse.json({
                success:false,
        ...
        const [data]:any = await pool.query('SELECT * FROM endpoints WHERE primary_id=?',[primary_id])
        if(data[0]?.name){
            let temp=''
            ...
            const [rowsss] = await DbPool.query(sql)
        }
        return NextResponse.json({
            success:true,
