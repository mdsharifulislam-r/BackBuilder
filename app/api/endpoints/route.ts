...
export async function POST(Request:Request) {
    try {
        const {name,project_id,schmea,is_user,is_auth_required}:{name:string,project_id:number,schmea:{name:string,type:string,required:boolean}[],is_user:boolean,is_auth_required?:string}=await Request.json()
       ...
       await pool.execute('CREATE TABLE IF NOT EXISTS endpoints (primary_id int not null AUTO_INCREMENT PRIMARY KEY,name varchar(256),project_id int,is_user boolean,is_auth_required varchar(256) )')
    //    await pool.execute('ALTER TABLE endpoints MODIFY COLUMN is_auth_required varchar(256) DEFAULT NULL')
       await pool.execute('CREATE TABLE IF NOT EXISTS scheme (schema_id int not NULL AUTO_INCREMENT PRIMARY KEY,primary_id INT,name varchar(256),type varchar(256),required boolean)')
        ...
        const makeEndPoint = 'INSERT INTO `endpoints`( `name`, `project_id`,`is_user`,`is_auth_required`) VALUES (?,?,?,?)'
        const [rows]=await pool.execute(makeEndPoint,[name,project_id,is_user,is_auth_required||false])
